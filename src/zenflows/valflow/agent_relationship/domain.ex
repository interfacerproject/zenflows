defmodule Zenflows.Valflow.AgentRelationship.Domain do
@moduledoc "Domain logic of AgentRelationships."

alias Ecto.Multi
alias Zenflows.Ecto.Repo
alias Zenflows.Valflow.AgentRelationship
...
@spec create(params()) :: {:ok, AgentRelationship.t()} | {:error, chset()}
def create(params) do
	Multi.new()
	|> Multi.insert(:rel, AgentRelationship.chset(params))
	|> Repo.transaction()
	|> case do
		{:ok, %{rel: rel}} -> {:ok, rel}
...
@spec update(id(), params()) :: {:ok, AgentRelationship.t()} | {:error, chset()}
def update(id, params) do
	Multi.new()
	|> Multi.run(:get, multi_get(id))
	|> Multi.update(:update, &AgentRelationship.chset(&1.get, params))
	|> Repo.transaction()
	|> case do
...
@spec delete(id()) :: {:ok, AgentRelationship.t()} | {:error, chset()}
def delete(id) do
	Multi.new()
	|> Multi.run(:get, multi_get(id))
